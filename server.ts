...
async function startServer() {
  ...
  app.get("/api/search", (req, res) => {
    ...
    // Simulate multi-platform search results
    const platforms = [
      ...
      { name: "Zomato", basePrice: 1.2, delivery: "₹45", rating: 4.1, urlPattern: "https://www.zomato.com/search?q=" },
    ...
    const results = platforms.map(p => {
      ...
      return {
        id: `${p.name}-${seed}`,
        name: `${query.charAt(0).toUpperCase() + query.slice(1)} ${isFood ? 'Special' : 'Pack'}`,
        platform: p.name,
        price,
        delivery: deliveryTime,
        rating: p.rating,
        image: `https://picsum.photos/seed/${query.replace(/\s+/g, '-')}-${p.name.toLowerCase()}/400/400`,
        link: `${p.urlPattern}${encodedQuery}`
      };
    });
